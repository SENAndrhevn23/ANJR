...
class NotesColorSubState extends MusicBeatSubstate
{
	...
	public function new() 
	{
		...
		var grid:FlxBackdrop = new FlxBackdrop(FlxGridOverlay.createGrid(80, 80, 160, 160, true, 0x1AFFFFFF, 0x0));
		grid.velocity.set(40, 40);
		grid.alpha = 0;
		FlxTween.tween(grid, {alpha: 1}, 0.5, {ease: FlxEase.quadOut});
		add(grid);

		// --- UI Panel Layout Containers ---
		var leftPanel:FlxSprite = new FlxSprite(36, 72).makeGraphic(670, 458, 0xFF1A2030);
		leftPanel.scrollFactor.set();
		add(leftPanel);

		var rightPanel:FlxSprite = new FlxSprite(720, 72).makeGraphic(524, 458, 0xFF1D2433);
		rightPanel.scrollFactor.set();
		add(rightPanel);

		var topBar:FlxSprite = new FlxSprite(36, 28).makeGraphic(1208, 34, 0xFF202738);
		topBar.scrollFactor.set();
		add(topBar);

		var footerBar:FlxSprite = new FlxSprite(36, 542).makeGraphic(1208, 38, 0xFF171B25);
		footerBar.scrollFactor.set();
		add(footerBar);

		var accentLine:FlxSprite = new FlxSprite(36, 65).makeGraphic(1208, 3, 0xFF6D7A9A);
		accentLine.scrollFactor.set();
		add(accentLine);

		// --- Dynamic Selection Background Highlights ---
		modeBG = new FlxSprite(215, 85).makeGraphic(315, 115, 0xFF232A38);
		modeBG.visible = false;
		modeBG.alpha = 0.6;
		add(modeBG);

		notesBG = new FlxSprite(140, 190).makeGraphic(480, 125, 0xFF232A38);
		notesBG.visible = false;
		notesBG.alpha = 0.6;
		add(notesBG);

		// --- Titles and Headers ---
		var menuTitle:FlxText = new FlxText(56, 36, 420, "NOTE COLORS", 24);
		menuTitle.scrollFactor.set();
		menuTitle.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, LEFT);
		add(menuTitle);

		var editorLabel:FlxText = new FlxText(1060, 36, 160, "Palette Editor", 16);
		editorLabel.scrollFactor.set();
		editorLabel.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT);
		add(editorLabel);
		...
		var ctrlLabel:Alphabet = new Alphabet(72, 92, 'CTRL', false);
		ctrlLabel.alignment = CENTERED;
		ctrlLabel.setScale(0.35);
		add(ctrlLabel);

		// --- Color Function Utility Elements ---
		copyButton = new FlxSprite(745, 100).loadGraphic(Paths.image('noteColorMenu/copy'));
		copyButton.alpha = 0.6;
		add(copyButton);

		pasteButton = new FlxSprite(1175, 100).loadGraphic(Paths.image('noteColorMenu/paste'));
		pasteButton.alpha = 0.6;
		add(pasteButton);

		colorGradient = FlxGradient.createGradientFlxSprite(60, 330, [FlxColor.WHITE, FlxColor.BLACK]);
		colorGradient.setPosition(750, 185);
		add(colorGradient);

		colorGradientSelector = new FlxSprite(740, 185).makeGraphic(80, 10, FlxColor.WHITE);
		colorGradientSelector.offset.y = 5;
		add(colorGradientSelector);

		colorPalette = new FlxSprite(820, 485).loadGraphic(Paths.image('noteColorMenu/palette', false));
		colorPalette.scale.set(16, 16);
		colorPalette.updateHitbox();
		colorPalette.antialiasing = false;
		add(colorPalette);
		
		colorWheel = new FlxSprite(845, 175).loadGraphic(Paths.image('noteColorMenu/colorWheel'));
		...
		// --- Numeric Alpha Value Indicators ---
		var txtX = 990;
		var txtY = 115;
		alphabetR = makeColorAlphabet(txtX - 90, txtY);
		add(alphabetR);
		alphabetG = makeColorAlphabet(txtX, txtY);
		add(alphabetG);
		alphabetB = makeColorAlphabet(txtX + 90, txtY);
		add(alphabetB);
		alphabetHex = makeColorAlphabet(txtX, txtY - 45);
		add(alphabetHex);

		hexTypeLine = new FlxSprite(0, 20).makeGraphic(5, 45, FlxColor.WHITE);
		...
		// --- Footer Layout Information Texts ---
		var tipX = 56;
		var tipY = 551;
		tip = new FlxText(tipX, tipY, 700, Language.getPhrase('note_colors_tip', 'Press RESET to Reset the selected Note Part.'), 16);
		tip.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT);
		add(tip);

		tipTxt = new FlxText(tipX + 450, tipY, 700, '', 16);
		tipTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT);
		add(tipTxt);
		updateTip();
	...
	override function update(elapsed:Float) 
	{
		...
		if(generalPressed)
		{
			...
			else if(pointerY() >= alphabetHex.y && pointerY() < alphabetHex.y + 50 && Math.abs(pointerX() - 990) <= 84)
			{
				hexTypeNum = 0;
	...
	public function spawnNotes()
	{
		...
		skinNote = new FlxSprite(48, 86).loadGraphic(Paths.image('noteColorMenu/' + (onPixel ? 'note' : 'notePixel')), true, res, res);
