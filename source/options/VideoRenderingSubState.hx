...
class VideoRenderingSubState extends BaseOptionsMenu
{
	...
	function deleteDirectoryWithFiles(path:String):Void
	{
		if (path == null || path == "")
			return;
		...
		if (sys.FileSystem.isDirectory(path))
		{
			for (file in sys.FileSystem.readDirectory(path))
			{
				...
				if (sys.FileSystem.isDirectory(fullPath))
					deleteDirectoryWithFiles(fullPath);
				else if (sys.FileSystem.exists(fullPath))
	...
	function testRender()
	{
		...
		var backupPreviewRender = ClientPrefs.data.previewRender;
		...
		deleteDirectoryWithFiles(video.target);
		FlxG.sound.play(Paths.sound('soundtray/VolMAX'), ClientPrefs.data.sfxVolume);
		ClientPrefs.data.codec = backupCodec;
		ClientPrefs.data.previewRender = backupPreviewRender;

		var o = curOption;
		o.setValue(backupPreviewRender);
		reloadCheckboxes();
