...
class PlayState extends MusicBeatState
{
	...
	private function resolveTotalColumns(?songData:Dynamic = null):Int
	{
		if (songData == null)
		...
		if (Reflect.hasField(songData, 'keyCount'))
		{
			var value:Dynamic = Reflect.field(songData, 'keyCount');
			if (value != null)
				return Std.int(value);
		}
		if (Reflect.hasField(songData, 'mania'))
		{
			var value:Dynamic = Reflect.field(songData, 'mania');
			if (value != null)
				return Std.int(value) + 1;
		}
		if (Reflect.hasField(songData, 'columns'))
		{
			var value:Dynamic = Reflect.field(songData, 'columns');
			if (value != null)
				return Std.int(value);
		}
		return 4;
	...
	private function syncKeyCount(?songData:Dynamic = null):Void
	{
		totalColumns = Std.int(Math.max(1, resolveTotalColumns(songData)));
		keysArray = [];
		var baseKeys:Array<String> = ['note_left', 'note_down', 'note_up', 'note_right'];
		for (i in 0...totalColumns)
			keysArray.push(i < baseKeys.length ? baseKeys[i] : 'note_' + i);
		while (strumsBlocked.length < totalColumns)
			strumsBlocked.push(false);
		if (strumsBlocked.length > totalColumns)
			strumsBlocked = strumsBlocked.slice(0, totalColumns);
	}

	private inline function isStrumBlocked(index:Int):Bool
	{
		return index < 0 || index >= strumsBlocked.length ? false : strumsBlocked[index];
	}

	private inline function isRightSideColumn(index:Int):Bool
	{
		return index >= Std.int(Math.ceil(totalColumns / 2));
	}
    ...
    public var showNotes:Bool = ClientPrefs.data.showNotes;
    public var showAfter:Bool = ClientPrefs.data.showAfter;
    public var keepNotes:Bool = ClientPrefs.data.keepNotes;
    public var sortNotes:String = ClientPrefs.data.sortNotes;
    public var fastSort:Bool = ClientPrefs.data.fastSort;
    public var betterRecycle:Bool = ClientPrefs.data.betterRecycle;
    public var limitNotes:Int = ClientPrefs.data.limitNotes;
    public var hideOverlapped:Float = ClientPrefs.data.hideOverlapped;
    public var processFirst:Bool = ClientPrefs.data.processFirst;
    public var skipSpawnNote:Bool = ClientPrefs.data.skipSpawnNote;
    public var bulkSkip:Bool = ClientPrefs.data.bulkSkip;
    public var breakTimeLimit:Bool = ClientPrefs.data.breakTimeLimit;
    public var optimizeSpawnNote:Bool = ClientPrefs.data.optimizeSpawnNote;
    public var noteHitPreEvent:Bool = ClientPrefs.data.noteHitPreEvent;
    public var noteHitEvent:Bool = ClientPrefs.data.noteHitEvent;
    public var noteHitStage:Bool = ClientPrefs.data.noteHitStage;
    public var skipNoteEvent:Bool = ClientPrefs.data.skipNoteEvent;
    public var spawnNoteEvent:Bool = ClientPrefs.data.spawnNoteEvent;
    public var skipArrowStartTween:Bool = false;
    public var disableGC:Bool = ClientPrefs.data.disableGC;
	...
	override public function create()
	{
		...
		if (!showNotes) cpuControlled = true;
...
public inline function formatScoreNumber(value:Int):String
{
	...
	private function startGameRenderer():Void
	{
		...
		renderGameMode = true;
		renderFPS = Std.int(Math.max(1, ClientPrefs.data.targetFPS > 0 ? ClientPrefs.data.targetFPS : ClientPrefs.data.videoRenderFPS));
		...
		try
		{
			...
			FFMpeg.instance.configure(SONG != null ? SONG.song : "unknown", renderFPS);
...
function startSong():Void
	{
		...
		renderGameMode = ClientPrefs.data.gameRenderer || ClientPrefs.data.previewRender;
		if (renderGameMode)
			set_playbackRate(1);
		@:privateAccess
	...
	private function createSpawnNote(noteData:Dynamic, ?parentNote:Note, ?prevNote:Note):Note
	{
		...
		{
			swagNote.x += 310;
			if(isRightSideColumn(swagNote.noteData))
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...totalColumns)
		{
			...
			else
			{
				...
				{
					babyArrow.x += 310;
					if(isRightSideColumn(i)) { //Up and Right
...
if(ClientPrefs.data.disableGC)
{
	...
	private function keyPressed(key:Int)
	{
		...
		var plrInputNotes:Array<Note> = notes.members.filter(function(n:Note):Bool {
			var canHit:Bool = n != null && !isStrumBlocked(n.noteData) && n.canBeHit && n.mustPress && !n.tooLate && !n.wasGoodHit && !n.blockHit;
		...
		if(!isStrumBlocked(key) && spr != null && spr.animation.curAnim.name != 'confirm')
		{
	...
	private function keysCheck():Void
	{
		...
		if(controls.controllerMode && pressArray.contains(true))
			for (i in 0...pressArray.length)
				if(pressArray[i] && !isStrumBlocked(i))
					keyPressed(i);
		if (startedCountdown && !inCutscene && !boyfriend.stunned && generatedMusic)
		{
			if (notes.length > 0) {
				for (n in notes) { // I can't do a filter here, that's kinda awesome
					var canHit:Bool = (n != null && !isStrumBlocked(n.noteData) && n.canBeHit
		...
		// TO DO: Find a better way to handle controller inputs, this should work for now
		if((controls.controllerMode || strumsBlocked.indexOf(true) != -1) && releaseArray.contains(true))
			for (i in 0...releaseArray.length)
				if(releaseArray[i] || isStrumBlocked(i))
					keyReleased(i);
	}
